...
def test_resolve_n_scalar_n():
    ...
    result = _resolve_n([S2Band.B02, S2Band.B01], n=100, extent_km=None)
...
def test_resolve_n_dict_n():
    ...
    result = _resolve_n([S2Band.B02, S2Band.B01], n=mapping, extent_km=None)
    assert result == mapping


def test_resolve_n_scalar_extent_km():
    """Derive n from a scalar extent_km using band resolution."""
    result = _resolve_n([S2Band.B02], n=None, extent_km=1.0)
    assert result[S2Band.B02] == round(1.0 / S2Band.B02.res_km)
...
def test_resolve_n_dict_extent_km():
    ...
    result = _resolve_n([S2Band.B02, S2Band.B01], n=None, extent_km=extents)
    assert result[S2Band.B02] == round(1.0 / S2Band.B02.res_km)
    assert result[S2Band.B01] == round(6.0 / S2Band.B01.res_km)


def test_resolve_n_both_raises():
    """Raise ValueError when both n and extent_km are provided."""
    with pytest.raises(ValueError, match="mutually exclusive"):
        _resolve_n([S2Band.B02], n=10, extent_km=1.0)


def test_resolve_n_none_raises():
    """Raise ValueError when neither n nor extent_km is provided."""
    with pytest.raises(ValueError, match="exactly one"):
        _resolve_n([S2Band.B02], n=None, extent_km=None)
...
def test_gaussian_image_dict_shape():
    ...
    scene = gaussian_image_dict(sigma=1.0, n=32, bands=[S2Band.B02])
...
def test_gaussian_image_dict_extent_km_shape():
    ...
    extent = 1.0
    scene = gaussian_image_dict(sigma=1.0, extent_km=extent, bands=[S2Band.B02])
    expected_n = round(extent / S2Band.B02.res_km)
...
def test_gaussian_image_dict_multiband_shape():
    ...
    scene = gaussian_image_dict(
        sigma=1.0, extent_km=1.0, bands=[S2Band.B02, S2Band.B01]
    )
    n_b02 = round(1.0 / S2Band.B02.res_km)
    n_b01 = round(1.0 / S2Band.B01.res_km)
...
def test_gaussian_center_equals_rho_max():
    ...
    scene = gaussian_image_dict(sigma=1e6, rho_min=0.0, rho_max=0.8, n=11, bands=[S2Band.B02])
...
def test_gaussian_boundary_approaches_rho_min():
    ...
    scene = gaussian_image_dict(sigma=0.001, rho_min=0.2, rho_max=1.0, n=11, bands=[S2Band.B02])
    da = scene[S2Band.B02]["rho_s"]
...
def test_disk_image_dict_shape():
    ...
    scene = disk_image_dict(radius=0.05, n=11, bands=[S2Band.B02])
...
def test_disk_image_dict_values():
    ...
    scene = disk_image_dict(radius=0.05, rho_min=0.0, rho_max=1.0, n=11, bands=[S2Band.B02])
    da = scene[S2Band.B02]["rho_s"]
...
def test_random_image_dict_shape():
    ...
    scene = random_image_dict(bands=[S2Band.B02], variables=["rho_s", "rho_toa"], n=16)
...
def test_random_image_dict_reproducible():
    ...
    s1 = random_image_dict(bands=[S2Band.B02], variables=["rho_s"], n=16, seed=42)
    s2 = random_image_dict(bands=[S2Band.B02], variables=["rho_s"], n=16, seed=42)
...
def test_random_image_dict_different_seeds():
    ...
    s1 = random_image_dict(bands=[S2Band.B02], variables=["rho_s"], n=16, seed=0)
    s2 = random_image_dict(bands=[S2Band.B02], variables=["rho_s"], n=16, seed=1)
